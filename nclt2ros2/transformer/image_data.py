...
class ImageData(BaseRawData, BaseConvert):
    ...
    def get_image_timestamps(self):
        ...
        timestamps_microsec = sorted(
            [long(os.path.splitext(f)[0]) for f in files if f.endswith(".tiff")]
        )
    ...
    def write_images(self, utime, cam_file):
        ...
        img_static_transform_stamped.header.stamp = timeMsg
        ...
        tf_static_msg.transforms = [img_static_transform_stamped]

        return int(utime) * 1000, img_msg, tf_static_msg
